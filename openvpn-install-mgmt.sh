...
#
# Original OpenVPN script Copyright (c) 2013 Nyr. Released under the MIT License.
#
...
new_client () {
	...
	echo "</tls-crypt>"
        ...
        # create random zip and Grav user password, compress and archive ovpn and copy to vpn website profile
        ZPASS=$(pwgen -s 12 1)
...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	...
	clear
	echo 'Welcome to this OpenVPN installer and management script!'
	...
	echo "   2) Google"
	echo "   3) 1.1.1.1 (Cloudflare private - recommended)"
	echo "   4) OpenDNS"
	echo "   5) Quad9"
	echo "   6) AdGuard"
...
LimitNPROC=infinity" > /etc/systemd/system/openvpn-server@server.service.d/disable-limitnproc.conf
	...
	# Get easy-rsa
	# added var to top of script - easy_rsa_url='https://github.com/OpenVPN/easy-rsa/releases/download/v3.2.1/EasyRSA-3.2.1.tgz'
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	echo
	echo "* To create serialized generic clients, run openvpn-create-next-client.sh *"
    echo
    echo
else
	clear
	...
	case "$option" in
		1)
			echo
			echo "Provide a name for the client/user:"
		...
		3)
			echo
			echo "*** You will LOSE all config, clients and users!! ***"
            echo
